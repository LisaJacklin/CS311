...
int main()
{
    ...
    ostream_iterator<int> out(cout, " "); //wrapper around the output
    cout << "Vector contents (printed using ostream_iterator):" << endl;
    copy(begin(data), end(data), out); //prints all data in data
    cout << endl << endl; 

    // Sort part of 1st vector & print again
    cout << "Sorting items 3-7 in vector using std::sort" << endl;
    sort(begin(data)+3, begin(data)+7+1); //specification of a range
