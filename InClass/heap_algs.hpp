...
{
    std::size_t size = last - first; //why not calculate the size for later calculations.

    //note that the slide algorithms are used to calcualte the following items
    for (std::size_t index = 1 index <size; ++index)
    {
        if (first[(index -1) /2] < first [indx])
            return false;
    }
...
{
    std::size_t size = last - first; //why not calculate the size for later calculations.
    assert (size != 0); //we better not be getting zero size here....just no

    std::size_t curr = size -1;
    while (curr != 0)
        {
            std::size_t parent = (curr -1) /2;
            if (!(first[parent] < first[curr])) //since we are doing an insert, the parameters are the oposite of delete above!
                break;
